...
import gradio as gr #Gradio 是一个开源 Python 库，可简化为机器学习模型创建 Web 界面的过程。它提供了一个简单直观的 API，用于构建交互式 UI，用户可以在其中输入数据，并实时显示模型的预测或输出。
import yaml
...
#上述代码设置了一个 Gradio 接口，用于配置和执行机器学习管道的各个步骤，包括数据预处理和模型训练。用户可以与界面交互以指定设置并启动不同的处理步骤。结果和进度显示在 Gradio 界面中。
        preprocess_button.click(#按钮触发的函数和输入输出的位置
